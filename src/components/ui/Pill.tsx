...
export function Pill({ displayName, userId, onClick }: PillProps) {
  ...
  return (
    <div className={`inline-flex items-center px-2.5 py-0.5 m-0.5 text-black dark:text-slate-800 font-medium rounded-full cursor-pointer ${color} active:scale-95 transition-all duration-200`} onClick={onClick}>
      {displayName}
      {unreadCount > 0 && (
